...
describe Typhoeus::Request::Before do
  ...
  describe '#queue' do
    context 'when before' do
      context 'when one' do
        it 'executes' do
          Typhoeus.before { |_r| receive_counter.mark }
        ...
        context 'when false' do
          it "doesn't call super" do
            ...
            expect(Typhoeus::Expectation).not_to receive(:response_for)
            request.run
          end

          it 'returns response' do
            Typhoeus.before do |r|
              r.response = 1
              false
            end
      ...
      context 'when multi' do
        context 'when all true' do
          before { 3.times { Typhoeus.before { |_r| receive_counter.mark } } }
        ...
        context 'when middle false' do
          before do
            Typhoeus.before { |_r| receive_counter.mark }
            Typhoeus.before do |_r|
              receive_counter.mark
              nil
            end
            Typhoeus.before { |_r| receive_counter.mark }
          end
          ...
          it 'executes only two' do
            expect(receive_counter).to receive(:mark).twice
            request.run
          end
        end
      end

...
describe 'Rack::Typhoeus::Middleware::ParamsDecoder::Helper' do
  ...
  describe '#decode' do
    ...
    it 'decodes' do
      expect(decoded[:array]).to match_array(%i[a b])
    end

    it "doesn't modify" do
      expect(decoded).not_to be(params)
    end
  end
      ...
      context 'when encoded' do
        ...
        context 'when longer and more complex' do
          ...
          it 'decodes ensuring arrays maintain their original order' do
            expect(decoded[:ids]).to eq(%w[407304 407305 407306 407307 407308 407309 407310 407311 407312 407313 327012])
          end
        end
  ...
  describe '#encoded?' do
    ...
    context 'when keys are ascending numbers starting with zero' do
      let(:params) { 12.times.map { |i| [i, (i + 65).chr] }.to_h }
  ...
  describe '#convert' do
    ...
    context 'when encoded' do
      ...
      it 'returns values' do
        expect(converted).to match_array(%i[a b])
      end
    end
